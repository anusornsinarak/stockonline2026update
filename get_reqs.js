...
async function run() {
    ...
    // Get ALL requisitions for this product to see what statuses they have
    const { data: reqItems } = await supabase.from('requisition_items')
        .select(`quantity, approved_quantity, requisitions!inner(status, created_at)`)
        .eq('product_id', pid);
        
    console.log("All req items for product:", reqItems.length);
    reqItems.forEach(r => console.log(r.requisitions.status, r.requisitions.created_at, r.quantity, r.approved_quantity));
}
run();
