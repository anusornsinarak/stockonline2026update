...
async function run() {
    ...
    // Simulate what StockCardView gets:
    // It calls supabaseService.getProductTransactionHistory
    ...
    console.log("RPC txs count:", txs.length); // Is this 0?
    ...
    console.log("Stock Card Sum Out:", sumOut);

    // Let's also check what product_transactions view directly gives
    const { data: vTxs } = await supabase.from('product_transactions').select('*').eq('product_id', pid);
    
    // Override logic on vTxs
    if (vTxs && reqItems) {
        const reqMap = new Map();
        reqItems.forEach((item) => {
            if (item.requisitions?.requisition_number && item.approved_quantity !== null) {
        ...
        vTxs.forEach(tx => {
            if (tx.transaction_type === 'เบิกจ่าย' && tx.reference_document && reqMap.has(tx.reference_document)) {
                tx.quantity_out = reqMap.get(tx.reference_document);
            }
        });
    }
    ...
    vTxs?.forEach(t => {
        ...
        if (d >= startDate && d <= endDate && t.transaction_type === 'เบิกจ่าย') {
            sumVOut += (t.quantity_out || 0);
        }
    });
    console.log("View with Override Sum Out:", sumVOut);
}
run();
