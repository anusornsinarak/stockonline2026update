...
async function run() {
    ...
    // Call RPC without end_date (if optional)
    let res = await supabase.rpc('get_product_transactions', { p_product_id: pid });
    console.log("RPC without end date:", res.data?.length, res.error);
    
    // Call RPC with far future
    res = await supabase.rpc('get_product_transactions', { p_product_id: pid, p_end_date: '2099-12-31' });
    console.log("RPC with far future date:", res.data?.length, res.error);
}
run();
