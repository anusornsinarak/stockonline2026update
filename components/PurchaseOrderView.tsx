...
import PurchaseRequestPrintView from './admin/PurchaseRequestPrintView';
import * as XLSX from 'xlsx';
import DownloadIcon from './icons/DownloadIcon';
import TrashIcon from './icons/TrashIcon';

// ... (The rest of the file content is the same as PurchaseOrderView.tsx provided in the prompt, just ensuring imports are correct as shown above)
...
// ... (CommitteeForm, PurchaseOrderPrintView, PurchaseOrderDetailModal, PurchasePlanner components remain largely same, just re-rendered with correct imports)

// ... (Main PurchaseOrderView component)
...
export const PurchaseOrderView: React.FC<PurchaseOrderViewProps> = (props) => {
    ...
    const [approvalFormToPrint, setApprovalFormToPrint] = useState<PurchaseOrder | null>(null);
    ...
    useEffect(() => {
        if (approvalFormToPrint) {
            const handleAfterPrint = () => { setApprovalFormToPrint(null); }
    ...
    const handleCreatePOs = async (basket: any) => {
        try {
            await supabaseService.createPurchaseOrders(basket);
            alert(`สร้างใบสั่งซื้อฉบับร่าง ${Object.keys(basket).length} ใบสำเร็จ!`);
            props.onDataChange();
            setViewMode('list');
        } catch (error) {
    ...
    const handleExport = () => {
        ...
        const detailsData = poWithDetails.map((po) =>
            (po.items || []).map(item => ({
    ...
    return (
        <div className="print-view-po">
            {/* ... (Content omitted for brevity, assuming standard render) */}
                        ...
                        <tbody className="dark:bg-slate-800">
                            {poWithDetails.map((po: PurchaseOrderWithDetails) => (
                                 <tr key={po.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                                    ...
                                    <td className="p-3 text-sm text-right">{po.totalValue.toLocaleString('th-TH', {style:'currency', currency: 'THB'})}</td>
