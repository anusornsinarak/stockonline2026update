...
async function run() {
    // Check if product_transactions is a table or a view
    const { data: cols } = await supabase.from('product_transactions').select('*').limit(0);
    console.log("Cols available:", cols);
}
run();
